...
public class DelayedMessageSender implements Runnable {
	...
	public void run() {
		...
		try {
			...
			//}
            /*if (messageToSend instanceof  LYTellMessage) {
				LYTellMessage msg = (LYTellMessage) messageToSend;
				System.out.println(msg.getLYSnapshotResult().getRecordedAmount());
				int sum = 0;
				for (Map.Entry<Integer, Pair> transaction : msg.getLYSnapshotResult().getTransactions().entrySet()){
				...
				System.out.println("total: " + (msg.getLYSnapshotResult().getRecordedAmount() + sum));
			}*/
