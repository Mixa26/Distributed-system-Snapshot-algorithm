...
public class AppConfig {
	...
	public static ArrayList<String> initiators;

	public static Map<String, Integer> initiatorsSnapshotNumber;
	...
	public static void readConfig(String configName){
		...
		try {
			...
			initiators.addAll(Arrays.asList(properties.getProperty("initiators").split(",")));
		} catch (NullPointerException e) {
			timestampedErrorPrint("Initiators property doesn't exist in the properties file. Exiting...");
			System.exit(0);
		}

		// Initiate all the initiators snapshot numbers to 0.
		initiatorsSnapshotNumber = new ConcurrentHashMap<>();
		for (String initiator : initiators) {
			initiatorsSnapshotNumber.put(initiator, -1);
		}
		...
		for (int i = 0; i < serventCount; i++) {
			...
			// Is this servent an initiator of snapshot?
			boolean isInitiator = initiators != null && initiators.contains(String.valueOf(i));

			ServentInfo newInfo = new ServentInfo("localhost", i, serventPort, neighborList, isInitiator);
