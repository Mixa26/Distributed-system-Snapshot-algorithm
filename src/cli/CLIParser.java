...
public class CLIParser implements Runnable, Cancellable {
	...
	public void run() {
		...
		while (working) {
			...
			// Here we're gonna check if the user
			// tried to do a snapshot on a servent
			// that's not a initiator.
			if (commandName.equals("bitcake_info") && !AppConfig.myServentInfo.isInitiator()) {
				AppConfig.timestampedErrorPrint("This servent is not an initiator. Can't start a snapshot.");
				continue;
			}
